...
/** 添加参数 */
- (void)rm_setParamByKey:(NSString *)key param:(NSString *)param
{
...
- (void)rm_postCompletionHandler:(void(^)(NSData *data, NSURLResponse *response, NSError *error))completionHandler
{
    ...
    if (_params) {
        ...
        NSError *error;
        if (![NSJSONSerialization isValidJSONObject:_params]) {
//            if (error) {
//                NSDictionary *userInfo = @{NSLocalizedFailureReasonErrorKey: NSLocalizedStringFromTable(@"The `parameters` argument is not valid JSON.", @"NetService", nil)};
//                error = [[NSError alloc] initWithDomain:@"com.alamofire.error.serialization.request" code:NSURLErrorCannotDecodeContentData userInfo:userInfo];
//            }
            return ;
        }
        
        NSData *jsonData = [NSJSONSerialization dataWithJSONObject:_params options:NSJSONWritingPrettyPrinted error:&error];
